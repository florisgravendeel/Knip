...
def get_opening_hours():
    calendar = CustomerCalendar.getOpeningHours()
    return jsonify({'opening_hours_dm': calendar["day_month_format"], 'opening_hours_fd': calendar["full_date_format"]})
...
if __name__ == '__main__':
    print(CustomerCalendar.getOpeningHours()['day_month_format'])
    app.run()
    #cur = mysql.connect().cursor()
    #cur.execute('select * from reservation')
    #r = [dict((cur.description[i][0], value) for i, value in enumerate(row)) for row in cur.fetchall()]
